...
def convert_xyt_to_rt(
    ...
    """
    Convert a Laser object from 'xyt' Cartesian geometry to 'rt' cylindrical
    geometry.

    The conversion samples the xyt envelope on a polar grid, then decomposes
    the azimuthal dependence into Fourier modes to populate the rt grid. Only
    the mode amplitudes up to ``n_azimuthal_modes`` are retained.

    Parameters
    ----------
    laser_xyt : lasy.laser.Laser
        A Laser object whose ``dim`` is ``'xyt'``. The object is not modified
        in place; a new Laser object is returned.

    nr : int
        Number of radial grid points in the output rt grid.

    n_azimuthal_modes : int, optional
        Number of azimuthal modes to retain (default: 1, i.e. azimuthally
        symmetric mode 0 only).

    n_theta_evals : int, optional
        Number of azimuthal sample points used when decomposing the xyt field
        into azimuthal modes. Must satisfy
        ``n_theta_evals >= 2 * n_azimuthal_modes - 1``.
        Defaults to ``2 * n_azimuthal_modes - 1`` (minimum sufficient number).
        Increase this for highly asymmetric fields to reduce aliasing.

    r_max : float, optional
        Maximum radial extent [m] of the rt grid. Defaults to half the
        transverse extent of the xyt box, i.e.
        ``min(x_max - x_min, y_max - y_min) / 2``.

    Returns
    -------
    laser_rt : lasy.laser.Laser
        New Laser object in ``'rt'`` geometry containing the converted field.

    Notes
    -----
    * The xyt envelope is assumed to be centred on the optical axis (x=0, y=0).
      If the beam is off-axis the conversion will be inaccurate for mode 0.
    * Only the first azimuthal mode (m=0) is populated when
      ``n_azimuthal_modes=1`` (default), which is exact for cylindrically
      symmetric fields and gives the azimuthal average otherwise.
    * The temporal axis ``lo_t``/``hi_t`` and ``nt`` are taken directly from
      the xyt grid, so the time resolution is preserved exactly.
    ...
    ...     profile=profile,
    ... )
    >>> laser_rt = convert_xyt_to_rt(laser_xyt, nr=50, r_max=50e-6)
    ...
    if r_max is None:
        x_half = (float(x_axis[-1]) - float(x_axis[0])) / 2.0
        y_half = (float(y_axis[-1]) - float(y_axis[0])) / 2.0
        r_max = min(x_half, y_half)
    ...
    r_axis = grid_rt.axes[0]

    if n_theta_evals is None:
        n_theta_evals = max(2 * n_azimuthal_modes - 1, 1)
    ...
    x_np = xp.asarray(x_axis)
    y_np = xp.asarray(y_axis)
    t_np = xp.asarray(t_axis)
    field_np = xp.asarray(field_xyt)

    interpolator_re = RegularGridInterpolator(
        (x_np, y_np, t_np),
        xp.real(field_np),
        method="linear",
        bounds_error=False,
    ...
    interpolator_im = RegularGridInterpolator(
        (x_np, y_np, t_np),
        xp.imag(field_np),
        method="linear",
        bounds_error=False,
    ...
    n_modes_total = 2 * n_azimuthal_modes - 1
    field_rt = xp.zeros((n_modes_total, nr, nt), dtype=xp.complex128)

    r_np = xp.asarray(r_axis)

    theta_np = xp.linspace(0.0, 2.0 * xp.pi, n_theta_evals, endpoint=False)

    envelope_polar = xp.zeros((n_theta_evals, nr, nt), dtype=xp.complex128)
    for i_theta, theta in enumerate(theta_np):
        x_pts = r_np[:, None] * xp.cos(theta) * xp.ones((1, nt))
    ...
    az_modes = xp.fft.ifft(envelope_polar, axis=0)

    field_rt[0] = az_modes[0]
    for m in range(1, n_azimuthal_modes):
        field_rt[m] = az_modes[m]
        if n_modes_total > 1:
            field_rt[n_azimuthal_modes + m - 1] = az_modes[-m]

    grid_rt.set_temporal_field(xp.asarray(field_rt))

    from lasy.laser import Laser

    laser_rt = object.__new__(Laser)
    laser_rt.dim = "rt"
    laser_rt.grid = grid_rt
    laser_rt.profile = laser_xyt.profile
    laser_rt.output_iteration = 0
